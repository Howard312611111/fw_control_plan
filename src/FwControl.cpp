...
FwControl::FwControl()
{
    ...
    hud_sub = nh.subscribe<mavros_msgs::VFR_HUD>
            ("mavros/vfr_hud", 10, &FwControl::hud_cb, this);
    ...
    fw_pose_sub = nh.subscribe<nav_msgs::Odometry>
            ...
            // ("/uav0/base_pose_ground_truth", 10, &FwControl::getFwPose, this);
...
void FwControl::getFwPose(const nav_msgs::Odometry::ConstPtr& pose)
{
    ...
    fwpos <<  pose->pose.pose.position.x,pose->pose.pose.position.y,pose->pose.pose.position.z;
}
...
void FwControl::SwitchFlightMode(std::string flightMode)
{
    ...
    while(ros::ok())
    {
        if( !current_state.armed &&
            ...
            if( arming_client.call(cmd_arm) &&
                cmd_arm.response.success){
                ROS_INFO("Vehicle armed : %s", ros::this_node::getName().c_str());
            }
            last_request = ros::Time::now();
        }
        else 
...
void FwControl::GoStraight()
{
    ...
    while(ros::ok())
    {
        cmd_pos.pose.position.x = fwpos[0]+dist;
        cmd_pos.pose.position.y = fwpos[1];
        cmd_pos.pose.position.z = fwpos[2];
        dist += 50;
