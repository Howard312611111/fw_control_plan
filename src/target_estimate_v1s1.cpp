...
#include <fw_control/Trajectory3D.h>
...
void qp_cb(const fw_control::Trajectory3D::ConstPtr& msg)
{
    qp1.acc[0] = msg->acc.x;
    qp1.acc[1] = msg->acc.y;
    qp1.acc[2] = msg->acc.z;
...
Eigen::Quaternionf car_q;

void car_cb(const nav_msgs::Odometry::ConstPtr& msg)
{
    car_q.x() = msg->pose.pose.orientation.x;
    car_q.y() = msg->pose.pose.orientation.y;
    car_q.z() = msg->pose.pose.orientation.z;
    car_q.w() = msg->pose.pose.orientation.w;
}
...
Eigen::MatrixXd dynamics(Eigen::MatrixXd sigma_state)
{
    ...
    ROS_INFO("x1_:%.3f x2_:%.3f", sigma_state(member_x1, 0), sigma_state(member_x2, 0));
...
/*
        if(i == 3)
        {
            ROS_INFO("x1:%.3f x2:%.3f x3:%.3f", x1, x2, x3);
        }

        if(i == 3)
        {
            ROS_INFO("x1dot:%.3f,  %.3f,  %.3f,  %.3f,  %.3f,  %.3f,  %.3f,  %.3f,  %.3f,  %.3f", vq_cam(0)*x3, vq_cam(2)*x1*x3, (dyn1.uav_velcam(2)*x1 - dyn1.uav_velcam(0))*x3, dyn1.pt_wvelcam(0)*x1*x2, dyn1.pt_wvelcam(1), dyn1.pt_wvelcam(1)*x1*x1, dyn1.pt_wvelcam(2)*x2, dyn1.uav_wvelcam(0)*x1*(x2+dyn1.camuav_pose(1)*x3), dyn1.uav_wvelcam(1)*(-(x3*dyn1.camuav_pose(2)+1)-x1*(x3*dyn1.camuav_pose(0)+x1)), dyn1.uav_wvelcam(2)*(x3*dyn1.camuav_pose(1)+x2));
            cout << "cam angular vel: " << dyn1.pt_wvelcam(0) << endl;
        }
*/
        ...
        {
            ROS_INFO("x1:%.3f x2:%.3f x3:%.3f", x1, x2, x3);
            ROS_INFO("x1_:%.3f x2_:%.3f", x1_, x2_);
            //cout << "rq: " << rq << " vq_gnd: " << vq_gnd << " vq_cam: " << vq_cam << endl;
        }


        if(measurement_flag && !qp_flag)
        {
            //cout << "constant model.--------" << endl;
            rq_ = rq + vq_gnd * dt;
            vq_gnd_ = vq_gnd;
        }
        /* unlock the assumption of constant model, use the QP trajectory to update UKF */
        if(measurement_flag && qp_flag)
        {
            //cout << "take acc into prediction.--------" << endl;
            rq_ = rq + vq_gnd * dt;
            vq_gnd_ = vq_gnd + qp1.acc * dt;
        }
        /* if the YOLO measurement is lost, use the QP trajectory to update UKF */
        if(!measurement_flag && qp_flag)
        {
            //cout << "Yolo lost.-------- QP enter!!" << endl;
            rq_ = rq + qp1.vel * dt;
            vq_gnd_ = vq_gnd + qp1.acc * dt;
        }
        if(!measurement_flag && !qp_flag)
    ...
    ROS_INFO("x1_:%.3f x2_:%.3f" ,predict_sigma_state(member_x1,0),predict_sigma_state(member_x2,0));
    ...
    if(!measurement_flag && qp_flag)
    {
        cout << "Yolo lost.--------QP enter___________" << endl;
    }
    if(!measurement_flag && !qp_flag)
    {
        cout << "constant model.--------" << endl;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber qp_sub = nh.subscribe<fw_control::Trajectory3D>("/target_qp1", 10, qp_cb);
    ros::Subscriber car_sub = nh.subscribe<nav_msgs::Odometry>("/prius/pose_ground_truth", 10, car_cb);
    // ros::Subscriber car_sub = nh.subscribe<nav_msgs::Odometry>("/wamv/base_pose_ground_truth", 10, car_cb);
    ...
    ros::Publisher car_pub = nh.advertise<geometry_msgs::Vector3>("/prius/estimation/velocity_body", 1);
    ...
    while(ros::ok())
    {
        //cout << "x(0) normal check: " <<  x(0) << endl;
        if( !isnormal(x(0)) )
        {
            initialize();
            x(0) = (cam1.u - cam1.cu) / cam1.fx;
...
*/
        ...
        */
        ...
        Eigen::Matrix3f car_rot;
        car_rot = Quat2RotaMatrix(car_q);
        Eigen::Vector3f car_vel, car_velgnd;
        car_velgnd << x(5), x(6), x(7);
        car_vel = car_rot*car_velgnd;

        geometry_msgs::Vector3 car_velbody;
        car_velbody.x = car_vel[0];
        car_velbody.y = car_vel[1];
        car_velbody.z = car_vel[2];


        gt_pub.publish(gt_value);
        estimate_pub.publish(estimate_value);
        car_pub.publish(car_velbody);
